...
const ArchivedChatRow: React.FC<{ item: ChatListItem }> = ({ item }) => {
  ...
  const formattedDate = format(new Date(item.last_message_at), "PPp", {
    locale: dateLocale,
  });
  ...
  return (
    ...
    >
      <Text style={s.rowText}>{`${t(
        "archivedChats.chatFrom"
      )} ${formattedDate}`}</Text>
      <Ionicons name="chevron-forward" size={20} style={s.rowIcon} />
    </Pressable>
  );
};
      ...
      const fetchArchivedChats = async () => {
        ...
        try {
          const response = await api.get<ChatListItem[]>(
            `/api/v1/chats/archived/bot/${botId}/`
          );
