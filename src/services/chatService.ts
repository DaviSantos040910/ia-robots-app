...
const realChatService = {
  ...
  // Atualizado para aceitar durationMs e enviá-lo ao backend
  async sendVoiceMessage(chatId: string, audioUri: string, durationMs: number, replyWithAudio: boolean = false): Promise<ChatMessage[]> {
      const formData = new FormData();
      // @ts-ignore
      ...
      // Envia a duração calculada pelo app para o backend
      formData.append('duration', String(Math.floor(durationMs)));
