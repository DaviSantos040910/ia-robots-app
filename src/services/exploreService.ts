...
const realExploreService = {
  ...
  async getBots(categoryId?: string): Promise<ExploreBotItem[]> {
    ...
    const qs = categoryId
      ...
      : "";
    const response = await api.get<ExploreBotItem[]>(
      `/api/v1/explore/bots/${qs}`
    );
    return response;
  },
  ...
  async searchBots(query: string): Promise<ExploreBotItem[]> {
    const response = await api.get<ExploreBotItem[]>(
      `/api/v1/explore/bots/?q=${encodeURIComponent(query)}`
    );
    return response;
  },
  ...
  async toggleBotSubscription(
    botId: string
  ): Promise<{ status: "subscribed" | "unsubscribed" }> {
    const response = await api.post<{ status: "subscribed" | "unsubscribed" }>(
      `/api/v1/bots/${botId}/subscribe/`
    );
    return response;
  },
};
...
export const exploreService = USE_MOCK_API
  ? mockExploreService
  : realExploreService;
